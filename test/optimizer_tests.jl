...
@testitem "optimizer" begin
    ...
    @testset "per-rule admission: traces, type fixed point, bitwise vs host over 3 steps" begin
        ...
        # `Descent` alone is not bitwise, because XLA contracts its `x - eta*g` into a fused
        # multiply-add. It gets a 1-ulp bound here and an EXACT assertion against the FMA reference in
        # the addendum below, so nothing is merely tolerated.
        fma_contracted = ("Descent",)
        for (name, chain) in admitted
            @testset "$name" begin
                ...
                if name in fma_contracted
                    @test maximum(abs.(xd .- xh)) <= eps(F)
                else
                    @test xd == xh
                end
            end
